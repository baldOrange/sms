...
//        session.setAttribute("ADD_USER_MSG", "添加用户 " + user.getUsername() + " 成功");
     ...
     * @return 重定向到userList.jsp
     */
    @RequestMapping("/resetSelectedUserVip")
    public String resetSelectedUserVip(@RequestParam("id") Integer id) {
        int resetSelectedUserId = userService.resetVipByUserId(id);
        System.out.println("resetSelectedUserId ==> " + resetSelectedUserId);
