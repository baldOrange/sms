...
import com.practice.sms.bean.to.Check;
import com.practice.sms.bean.to.ChecKoutSale;
import com.practice.sms.service.*;
...
//        System.out.println("*********" + products);
    ...
    public String checkService(HttpServletRequest request, Model model, HttpSession session) {
    ...
    public String checkServicePlusPro(HttpServletRequest request, Model model) {
        // 会员信息
        Vip vip = vipService.getVipByVipId(1);
...
//            generateSaleItemAndUpdateStock(productId, checkObj.getProductNumber());
    ...
    /**
     * 保存销售服务(PlusPro版-2021年5月2日15:22:50)
     *
     * @param request 原生requestAPI, 用于获取表单数据
     * @param model 存储结算页面的抽象后封装的TO对象, 并放入request域中
     * @return checkout.jsp页面
     */
    @RequestMapping("/addProductSale")
    public String addProductSale(HttpServletRequest request, Model model) {
        Integer productId = Integer.parseInt(request.getParameter("productId"));
        Product product = productService.getProductByProductId(productId);
        //封装结算商品
        ChecKoutSale checKoutSale = new ChecKoutSale();
        checKoutSale.setProduct(product);
        checKoutSale.setProductId(productId);
        checKoutSale.setProductNumber(Integer.parseInt(request.getParameter("productNumber")));
        checKoutSale.setIsVip(request.getParameter("isVip"));
        checKoutSale.setCheckPrice(Float.parseFloat(request.getParameter("payableMoney")));
        checKoutSale.setGenerateTime(new Date());
        checKoutService.addProductSale(checKoutSale);
        return "other/checkout";
    }
